...
const optionalWorkInputSchema = z.object({
  ...
  price_ex_vat: z.number().min(0).nullable().optional().describe("Eenmalige meerprijs exclusief btw. null of weglaten = geen eenmalige prijs (bijv. puur een abonnement)."),
  recurring_price_ex_vat: z.number().min(0).nullable().optional().describe("Terugkerende prijs exclusief btw (abonnement, onderhoud). Zet hier het bedrag, niet in de tag of titel."),
  recurring_interval: z.enum(["maand", "jaar"]).nullable().optional().describe("Interval van de terugkerende prijs."),
  vat_rate: z.number().default(21),
  default_selected: z.boolean().optional().describe("true = in het klantportaal standaard aangevinkt (maar afvinkbaar)."),
...
function normalizeOptionalWork(options: z.infer<typeof optionalWorkInputSchema>[]) {
  return options.map((option) => ({
    ...
    d: option.summary,
    tag: option.tag,
    price: option.price_ex_vat ?? null,
    recurringPrice: option.recurring_price_ex_vat ?? null,
    recurringInterval: option.recurring_interval ?? null,
    vatRate: option.vat_rate,
    defaultSelected: option.default_selected ?? false,
...
function createMcpServer() {
  ...
  server.tool(
    ...
    "Maak een volledige offerte volgens hetzelfde contract als de plakimport. items bevat alleen de vaste basis. Zet volledige alternatieve systemen in configurations en selecteerbaar meerwerk in optional_work. Dubbel prijzen nooit tussen items, configurations en optional_work. Maak geen placeholders of verzonnen keuzes.",
    {
      ...
      optional_work: z.array(optionalWorkInputSchema).optional().describe("Selecteerbare modules / optioneel meerwerk. Eenmalige prijs in price_ex_vat, abonnement/onderhoud in recurring_price_ex_vat + recurring_interval. default_selected voor modules die standaard aanstaan. Laat leeg als er geen echte uitbreiding is."),
