...
import { donnaCompanyIds, donnaResponse, parseLimit, quoteDto } from "@/lib/donna";
...
export async function GET(req: NextRequest) { return donnaRoute(req, async () => {
  ...
  const quotes = await prisma.quote.findMany({ where: { companyId: { in: await donnaCompanyIds() }, ...(includeArchived ? {} : { archivedAt: null }), ...(customerRef ? { customerId: customerRef } : {}), ...(status ? { status: status as "DRAFT" } : {}) }, orderBy: { updatedAt: "desc" }, take: limit });
