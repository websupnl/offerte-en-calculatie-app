...
async function getDefaultUserForCompany(companyId) {
...
async function generateQuoteNumber(companySlug) {
...
server.tool("list_companies", "List available companies/brands. Use companySlug when creating quotes.", {}, async () => {
...
server.tool("list_customers", "List customers, optionally scoped to a company slug.", {
...
server.tool("upsert_customer", "Create or update a customer. Use this before creating a quote when the customer does not exist.", {
    ...
    if (!company)
        return text(`Company not found: ${companySlug}`);
    const customer = id
        ? await prisma.customer.update({ where: { id }, data })
        : await prisma.customer.create({ data: { ...data, companyId: company.id } });
    return json(customer);
});
server.tool("list_products", "List active products/articles that can be used as quote line items.", {
    companySlug: z.string().default("koolhaas"),
    category: z.string().optional(),
    query: z.string().optional(),
...
server.tool("list_product_sets", "List product bundles/sets with their products and quantities.", {
    companySlug: z.string().default("koolhaas"),
    category: z.string().optional(),
    includeInactive: z.boolean().default(false),
...
server.tool("create_quote", "Create a quote with all template fields and line items. Supports WebsUp and Koolhaas via companySlug.", {
    companySlug: z.string().default("koolhaas"),
    ...
    if (!user)
        return text("No user found to assign as quote creator.");
    const customer = await prisma.customer.findFirst({ where: { id: customerId, companyId: company.id } });
    if (!customer)
        return text(`Customer not found for ${companySlug}: ${customerId}`);
...
server.tool("get_quote", "Get a complete quote including customer, company, line items, share token and advice documents.", {
    id: z.string().min(1),
}, async ({ id }) => {
    const quote = await prisma.quote.findUnique({ where: { id }, include: quoteInclude() });
    if (!quote)
        return text(`Quote not found: ${id}`);
    return json({
        quote,
        dashboardUrl: `${appUrl}/quotes/${quote.id}`,
        printUrl: `${appUrl}/print/quotes/${quote.id}`,
...
server.tool("search_quotes", "Search quotes by customer, quote number, project title, status or company.", {
    query: z.string().optional(),
    companySlug: z.string().optional(),
...
server.tool("update_quote", "Update quote text/template fields, status, validUntil and optionally replace all line items.", {
    id: z.string().min(1),
...
server.tool("delete_quote", "Delete a quote and all its line items/share data.", {
    id: z.string().min(1),
}, async ({ id }) => {
    await prisma.quote.delete({ where: { id } });
    return text(`Deleted quote ${id}`);
});
server.tool("share_quote", "Create or fetch the customer portal share URL for a quote and mark DRAFT quotes as SENT.", {
    id: z.string().min(1),
}, async ({ id }) => {
...
server.tool("set_quote_status", "Set quote status, for example DRAFT, SENT, ACCEPTED or DECLINED.", {
    id: z.string().min(1),
...
server.tool("list_advice_documents", "List AI advice documents linked to a quote.", {
...
server.tool("get_quote_context", "Get everything Claude needs to reason about quote creation: companies, customers, products, sets and recent quotes.", {
    companySlug: z.string().default("koolhaas"),
